...
export default function Modal({modalRef, updateNote}){
    const titleRef = useRef(null)
    const textRef = useRef(null)
    const hideModalAnimationDuration = 300

    useEffect(() => {
        modalRef.current.hideModalAnimationDuration = hideModalAnimationDuration

        modalRef.current.openModal = (id, title, text) => {
            modalRef.current.noteId = id
            modalRef.current.querySelector('input').value = title
            modalRef.current.querySelector('textarea').value = text
            ...
            modalRef.current.animate(
                ...
                {
                  fill: "forwards",
                  duration: modalRef.current.hideModalAnimationDuration,
                  ease: 'ease-in-out'
                },
            );
        }
        modalRef.current.closeModal = () => {
            ...
            setTimeout(() => {
                modalRef.current.noteId = null
                modalRef.current.close()
            }, modalRef.current.hideModalAnimationDuration)
        }
    }, [])
    ...
    return (
        <dialog ref={modalRef} className="relative w-4/5 md:w-3/4 lg:w-1/2 bg-white dark:bg-neutral-900 shadow-md border dark:hover:border-neutral-700 dark:focus-within:border-neutral-700 border-neutral-300 p-4 rounded focus-within:border-neutral-400 hover:border-neutral-400 dark:border-neutral-800 overflow-hidden ">
            <div className='bg-white dark:bg-neutral-900 flex flex-col gap-1'>
                <input ref={titleRef} onChange={updateNoteModal} defaultValue={'XD'} type="text" placeholder="Title..." spellCheck="false" autoComplete="off" className="bg-transparent dark:placeholder:text-neutral-800 placeholder:italic text-2xl text-neutral-800 dark:text-white focus:outline-none font-semibold"/>
                <textarea ref={textRef} onChange={updateNoteModal} rows="15" placeholder="Text..." spellCheck="false" autoComplete="off" className="bg-transparent dark:placeholder:text-neutral-800 placeholder:italic text-xl text-neutral-800 dark:text-white focus:outline-none resize-none scrollbar-thin dark:scrollbar-thumb-neutral-700 scrollbar-thumb-neutral-400 scrollbar-track-transparent"></textarea>
